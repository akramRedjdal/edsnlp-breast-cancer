...
def log(obj='', level=DEBUG, file=None):
    """Minimal stand-in for the original project's Flask-coupled
    claritynlp_logging.log — this package has no Flask app context, so
    messages just go through the standard logging module instead."""
    if level == ERROR:
        _logger.error(repr(obj))
    else:
        _logger.debug(repr(obj))
...
_strUnits = r'(mm[\.23]?|cm[\.23]?|' + _str_in_meas      +\
           r'|cc\.?|millimetres?|centimetres?|inch|inches)'
...
_str_list_item = _x + r'\s*+[-,]?+(\s*+et\s*+)?+'
_str_list = r'(?<=\s)(' + _str_list_item + r'\s*+)*+' + _x +\
    r'\s*+[-,]?\s*+et\s*+' + _x + r'\s*+[-,]?\s*+' + _cm
...
def _tokenize_complete_list(sentence, list_text, list_start):
    ...
    list_text = re.sub(r'et', r',  ', list_text)
...
def run(sentence):
    ...
    while more_to_go:
        ...
        if -1 != best_regex_index:
            ...
            if 0 == len(s):
                break
    # filtre les mesures aberrantes >=100cm ; list-comp (evite le bug de
    # mutation pendant iteration qui sautait des elements)
    measurements = [i for i in measurements
                    if not re.search(r'\d{3}\s*cm', i[0], re.IGNORECASE)]
    return measurements
